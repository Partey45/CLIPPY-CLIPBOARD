...
import sys, json, uuid, os, csv, io, threading, time
...
# STATE
# ══════════════════════════════════════════════════════════════════════════════
def _gid(): return uuid.uuid4().hex[:7]
...
class Bridge(QObject):
        ...
        for row in state["rows"]:
            for e in row["entries"]:
                if e["id"] == entry_id:
                    e["copy_count"] = e.get("copy_count",0)+1
        save()
        # Pass the exact text to the window — it handles clipboard + paste atomically
    ...
    def addRow(self):
        ...
        state["rows"].append(r)
        # FIX #2: new group capture starts at the new group's first slot
        ri = len(state["rows"]) - 1
    ...
    def setAutoCapture(self, enabled):
        ...
        if not enabled: state["cursor"] = {"row_idx":-1,"entry_idx":-1}
        self._push()
    ...
    def clearHistory(self):
        state["history"] = []; self._push()

    @pyqtSlot()
    def exportJSON(self):
        path,_ = QFileDialog.getSaveFileName(None,"Save JSON","clippy-export.json","JSON (*.json)")
        if path:
            Path(path).write_text(json.dumps(state["rows"],ensure_ascii=False,indent=2),"utf-8")
    ...
    def hideWindow(self):
        ...
        if not getattr(_win_ref[0], '_pasting', False):
            _win_ref[0].hide()
    ...
    def importJSON(self):
        ...
        if path:
            try:
                ...
                state["rows"] = data; save(); self._push()
...
class ClipboardPoller(QThread):
    ...
    def run(self):
        # FIX #4: We track what we last PLACED into the clipboard ourselves.
        # The poller skips text only if it equals what Clippy placed AND
        # less than 1.5s have passed (echo prevention).
        # After 1.5s, re-copying the same word by the user is detected again.
        # "last_seen" is what we last read from the OS clipboard.
        # When the user DELETES and re-copies, pyperclip returns empty briefly
        # then returns the word again — we must detect that transition.
        last_seen = ""

        while True:
            time.sleep(0.5)
            if not state["auto_capture"] or not CLIPBOARD_OK:
                continue

            # Never capture while Clippy itself is writing to the clipboard
            # (i.e. during a paste operation triggered by Enter)
            ...
            try:
                ...
                # Empty clipboard — reset last_seen so next copy is fresh
                if not text:
                    last_seen = ""
                    continue

                # Same as what we already processed — skip (no change)
                if text == last_seen:
                    continue

                # When poller unfreezes after a paste, the clipboard still holds
                # the pasted text. Treat it as already seen so it is never captured.
                if text == _clippy_last_pasted:
                    last_seen = text  # mark as seen so future checks skip it too
                    continue

                # ── New text detected — user did a real Ctrl+C ────────────────
                last_seen = text

                # History
                if state["history_enabled"]:
                    h = [t for t in state["history"] if t != text]
                    h.insert(0,text); state["history"] = h[:10]

                # ── Place captured text into the correct slot ─────────────
                #
                # RULE: Stay on the current row and auto-add variation slots
                # until the row reaches 4 entries (no more Add Variation shown).
                # Only then move to the next row. User clicking a card overrides
                # everything via setCursorToEntry (handled separately).
                #
                rows = state["rows"]
                c    = state["cursor"]
                ri   = c["row_idx"]
                ...
                if ri < 0 or ri >= len(rows):
                    # Cursor points past all rows — create a brand new group
                    rows.append({"id":_gid(),"color_idx":len(rows),"entries":[_entry(text)]})
                    ri = len(rows) - 1
                    ei = 0
                else:
                    ...
                    if ei < len(entries):
                        # Slot exists — fill it directly
                        entries[ei] = {**entries[ei], "text": text}

                    elif len(entries) < 4:
                        # Slot doesn't exist yet but row has room (< 4 entries)
                        # Auto-add a new variation slot and fill it
                        entries.append(_entry(text))
                        ei = len(entries) - 1

                    else:
                        # Row is exhausted (4 entries, no Add Variation) — next row
                        ri += 1
                        if ri >= len(rows):
                            rows.append({"id":_gid(),"color_idx":len(rows),"entries":[_entry(text)]})
                            ei = 0
                            ...
                            ne = rows[ri]["entries"]
                            # Find first truly empty text slot
                            fi = next((i for i,e in enumerate(ne) if not e["text"]), None)
                            if fi is not None:
                                ne[fi] = {**ne[fi], "text": text}; ei = fi
                            elif len(ne) < 4:
                                # Next row has room — auto-add variation
                                ne.append(_entry(text)); ei = len(ne) - 1
                            else:
                                # Next row also exhausted — create new group
                                ri += 1
                                rows.append({"id":_gid(),"color_idx":len(rows),"entries":[_entry(text)]})
                                ei = 0

                # Advance cursor using the new slot-aware function
...
class ClippyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        tray_menu  = QMenu()
        tray_menu.addAction("Show Clippy", self._show_window)
        ...
        # Global hotkey — suppress=True so Ctrl+D is not passed through to other apps
        self.showRequested.connect(self._show_window)
        if HAS_KEYBOARD:
            kb_lib.add_hotkey("ctrl+d", lambda: self.showRequested.emit(), suppress=True)

        _win_ref[0] = self  # give Bridge slots access to this window
        load()
        self.web.setHtml(BUILD_UI(), QUrl("about:blank"))
        self._page_ready = False
        # Mark page ready 4s after load (gives JS bridge time to connect)
        QTimer.singleShot(4000, lambda: setattr(self, '_page_ready', True))

        # ── Layer 1: Keep-alive ping every 25s ───────────────────────────────
        # Prevents Chromium from suspending renderer during long idle periods
        ...
        # Detects blank page and reloads automatically
    ...
    def _show_window(self):
        ...
        # After long idle the renderer may have died — detect and fix first
        if getattr(self, '_page_ready', False):
            ...
            return
        # Page not ready yet (first launch) — show directly
        self._show_after_check('ok')

    def _show_after_check(self, health):
        if health != 'ok':
            # Page is blank — reload then show once ready
            self._reload_ui()
    ...
    def _do_show(self):
        if self._first_show:
            ...
            # Appear near mouse cursor, clamped to screen
            mouse_pos = QCursor.pos()
            tx = mouse_pos.x() + 15
            ty = mouse_pos.y() + 15
            screen = QApplication.primaryScreen().geometry()
            if tx + self.width()  > screen.width():  tx = screen.width()  - self.width()  - 10
            if ty + self.height() > screen.height(): ty = screen.height() - self.height() - 10
            self.move(tx, ty)

        # Show and restore from minimised
        self.show()
        self.setWindowState(self.windowState() & ~Qt.WindowState.WindowMinimized)
        self.raise_()
        self.activateWindow()

        # Force to front using Windows API — no flag toggling, no focus flicker
        self._force_foreground()

        # Focus search bar and arm keyboard nav
    ...
    def _force_foreground(self):
        """Use ctypes to force Clippy to the foreground on Windows."""
        try:
            import ctypes
            hwnd = int(self.winId())
            ...
            pass  # Non-Windows or ctypes not available — graceful fallback
    ...
    def _do_paste_and_hide(self, text):
        ...
        if HAS_KEYBOARD and CLIPBOARD_OK:
            def _paste():
                global _clippy_is_pasting, _clippy_last_pasted
                try:
                    # Wait for destination app to regain focus
                    time.sleep(0.4)
                    # Record what we are about to paste BEFORE writing to clipboard
                    # so the poller knows to skip it when it unfreezes
                    _clippy_last_pasted = text
                    # Write exact selected text right before keystroke — no gap
                    pyperclip.copy(text)
                    time.sleep(0.05)
                    # Ctrl+V — destination receives exactly `text`
                    kb_lib.send("ctrl+v")
                    # Stay frozen 1.5s so paste echo has time to die out
                    time.sleep(1.5)
                except:
                    pass
                finally:
...
html,body{height:100%;font-family:'Inter',sans-serif;margin:0;padding:0;}
...
/* ── DAYLIGHT ── */
body.daylight{
  --appBg:#f4f2ff;--navBg:rgba(244,242,255,.98);--navBorder:rgba(139,92,246,.12);
  --panelBg:#fff;--panelBorder:rgba(139,92,246,.18);
  --rowBg:rgba(255,255,255,.85);--rowBorder:rgba(139,92,246,.16);
  --cardBg:rgba(255,255,255,.92);--cardBorder:rgba(139,92,246,.16);
  --text:#1a0e40;--textDim:#6b7280;--textMuted:#a0aec0;
  --inputBg:rgba(255,255,255,.9);--inputBorder:rgba(139,92,246,.22);
  --sectionLabel:#9ca3af;--emptyText:#c4b5e8;
  --toastBg:rgba(255,255,255,.97);--toastBorder:rgba(139,92,246,.3);
...
/* ── NAV ── */
#nav{
  display:flex;align-items:center;padding:0 10px;height:52px;gap:10px;
...
#search{
  ...
  color:var(--text);font-size:12px;outline:none;font-family:inherit;transition:border-color .2s;
}
#search:focus{border-color:rgba(139,92,246,.5);}
...
.btn-new{
  flex-shrink:0;background:linear-gradient(135deg,#6d28d9,#a855f7);
  border:none;border-radius:7px;color:#fff;padding:6px 12px;
  font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;
  box-shadow:0 3px 10px rgba(109,40,217,.4);display:flex;align-items:center;gap:4px;transition:opacity .15s;
}
.btn-new:hover{opacity:.85;}

.btn-gear{
  flex-shrink:0;width:32px;height:32px;border-radius:7px;cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  background:var(--inputBg);border:1px solid var(--inputBorder);transition:all .18s;
}
.btn-gear svg{stroke:var(--gearStroke);transition:stroke .3s;}
.btn-gear.active,.btn-gear:hover{background:rgba(124,58,237,.18);border-color:rgba(139,92,246,.4);}

/* ── MAIN ── */
/* scroll-wrap fills remaining height below nav — rows scroll inside here */
#scroll-wrap{
  height:calc(100vh - 52px);  /* exactly viewport minus nav bar */
  overflow-y:auto;
  overflow-x:hidden;
...
#main{padding:14px 10px 60px;display:flex;flex-direction:column;gap:9px;}
.empty{text-align:center;color:var(--emptyText);padding-top:60px;font-size:13px;}

/* ── ROW ── */
.row-wrap{display:flex;align-items:stretch;gap:7px;transition:opacity .15s;min-height:123px;}
...
.row-group{
  ...
  border-radius:14px;padding:9px 11px 9px;
  display:flex;flex-direction:row;flex-wrap:nowrap;gap:8px;align-items:stretch;
  overflow:hidden;
  transition:border-color .25s,background .25s;
}
.row-group.active-capture{background:rgba(109,40,217,.04);border-color:rgba(139,92,246,.22);}
...
.row-del{
  ...
  z-index:2;font-size:8px;transition:all .15s;
}
.row-del:hover{background:rgba(239,68,68,.18);border-color:rgba(239,68,68,.4);color:#f87171;}

/* ── CARD ── */
.card{
  flex:1;min-width:0;height:105px;flex-shrink:1;position:relative;
...
.card:hover{
  border-color:rgba(139,92,246,.55);background:rgba(109,40,217,.08);
  box-shadow:0 0 0 1px rgba(139,92,246,.18),0 4px 12px rgba(109,40,217,.1);
}
...
.card-text.code-font{font-family:'Fira Code','Courier New',monospace;font-size:10.5px;}
...
.card-textarea{width:100%;height:100%;background:transparent;border:none;color:var(--text);font-size:12px;line-height:1.48;padding:0;outline:none;resize:none;font-family:'Fira Code','Courier New',monospace;}
...
/* ── ADD VARIATION ── */
.card-add{
  flex:1;min-width:0;height:105px;flex-shrink:1;cursor:pointer;
  background:rgba(109,40,217,.025);border:1.5px dashed rgba(139,92,246,.22);
...
.card-add:hover{background:rgba(109,40,217,.09);border-color:rgba(139,92,246,.5);}
...
.hist-item{width:100%;background:transparent;border:none;border-bottom:1px solid var(--navBorder);padding:7px 11px;text-align:left;color:var(--text);font-size:10px;cursor:pointer;font-family:'Fira Code','Courier New',monospace;display:flex;justify-content:space-between;align-items:center;gap:7px;}
  ...
  <div class="logo">
    <div class="logo-mark">
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <rect x="2" y="4" width="12" height="11" rx="2" fill="white" fill-opacity="0.9"/>
        <rect x="5.5" y="1.5" width="5" height="4" rx="1.5" fill="#7c3aed"/>
        <circle cx="8" cy="3.5" r="1" fill="white" fill-opacity="0.8"/>
        <rect x="4" y="7.5" width="8" height="1.2" rx="0.6" fill="#7c3aed"/>
        <rect x="4" y="9.8" width="8" height="1.2" rx="0.6" fill="#7c3aed"/>
        <rect x="4" y="12.1" width="5" height="1.2" rx="0.6" fill="#7c3aed"/>
      </svg>
    </div>
  ...
  <div class="settings-body">
    ...
    <div class="settings-section">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle-switch on" id="auto-toggle" onclick="toggleCapture()">
    ...
    <div class="settings-section">
      ...
      <div class="toggle-row">
        ...
        <div class="toggle-switch on" id="hist-toggle" onclick="toggleHistory()">
          <div class="toggle-thumb"></div>
        </div>
      </div>
...
// This is the ONLY auto-hide trigger — no Python changeEvent involved
window.addEventListener('blur', function() {
  // Small delay so internal focus moves (e.g. opening a dialog) don't hide
  setTimeout(function() {
...
function flatEntries() {
  ...
  const visible = searchTerm
    ? sorted.filter(r=>r.entries.some(e=>e.text.toLowerCase().includes(searchTerm)))
    : sorted;
  visible.forEach((row)=>row.entries.forEach((e,ei)=>out.push({ei,e,rowId:row.id})));
  return out;
}
    ...
    const cards = row.entries.map(entry => {
      ...
      if (editingId===entry.id) {
        body = `<div style="flex:1;overflow:hidden"><textarea class="card-textarea" id="ta-${entry.id}"
          ...
          oninput="syncTA('${entry.id}',this.value)">${escHtml(entry.text)}</textarea></div>
...
function addRow(){ bridge.addRow(function(json){ const d=JSON.parse(json); editingId=d.new_entry_id; }); }
...
function onSearch(val){
  ...
  // FIX #2: auto-highlight first match in kb nav so Enter pastes it
  if (searchTerm) {
    const flat=flatEntries();
    const mi=flat.findIndex(f=>f.e.text.toLowerCase().includes(searchTerm));
    kbIdx = mi >= 0 ? mi : -1;
  } else {
    kbIdx = -1;
  }
  ...
  if (searchTerm){
    const m=flatEntries().filter(f=>f.e.text.toLowerCase().includes(searchTerm));
    inp.placeholder=m.length?`${m.length} · Enter to paste`:'No matches';
  } else { inp.placeholder='Search entries…'; }
}
function clearSearch(){document.getElementById('search').value='';onSearch('');}
function onSearchKey(e){
  if (e.key!=='Enter') return;
  // FIX #2: Enter on search = paste first highlighted result to destination
  const flat=flatEntries();
  const m=flat.find(f=>f.e.text.toLowerCase().includes(searchTerm));
  if (m && m.e.text) {
    clearSearch();
    bridge.pasteEntry(m.e.id, m.e.text);
  }
}
...
document.addEventListener('keydown', e => {
  ...
  // Arrow keys work even when search is focused — navigate cards
  // Enter works from search (paste highlighted) or nav mode
  const isSearch = document.activeElement?.id==='search';

  // Only process nav keys when in nav mode OR search is active with a match
  ...
  if (e.key==='ArrowRight') {
    ...
    renderAll();
    document.getElementById('card-'+(flat[kbIdx]?.e?.id))?.scrollIntoView({block:'nearest',behavior:'smooth'});
...
if __name__ == "__main__":
    ...
    # Prevent WebEngine renderer crashes and white screens
    os.environ.setdefault("QTWEBENGINE_CHROMIUM_FLAGS",
        "--disable-gpu --disable-software-rasterizer "
        "--disable-gpu-sandbox --no-sandbox --disable-dev-shm-usage")
